...
private:
	...
	static const int TCHILD_MAX = KEY_MAX+1;
...
private:  
    ...
    //查找关键字  
	bool search(Node *pNode, const T &key)   
	{  
    ...
    //用括号打印树  
    void displayInConcavo(Node *pNode, int count)
    {  
    	if (pNode!=NULL)  
    	{  
    		int i, j;  
...
int main()
{
	int list[10]={2,6,42,5,7,8,3,5,12,1};
	Btree<int> tree;
	for (int i = 0; i < 10; ++i)
	{
		tree.insert(list[i]);
